...
const totalSum = (input) => {
  return input.map(arr => arr.reduce((acc, curr) => acc + curr)).reduce((acc, curr) => acc + curr);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => arr.filter(num => { return typeof num === 'number' && num % 5 === 0 }).map(num => Math.pow(2, num)));
};
...
let findMaleAndFemale = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (data[i].gender === 'male' || data[i].gender === 'female') {
      if (i !== data.length - 1)
        newData += `${data[i].name} and `;
      else newData += `${data[i].name}`;
    }
  }
  return newData;
...
let findShortest = (data) => {
  let findShortest = (data) => {
    let shortest = data.map(arr => arr.height).sort((a, b) => a - b);
    return data.find(arr => arr.height === shortest[0]).name;
  };};

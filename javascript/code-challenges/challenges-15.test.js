...
let biggerThanLuke = (arr) => {
  let filtered = arr.filter(obj => obj.mass > 77);
...
const sortBy = (property, arr) => {
  arr.sort((a,b) => (a[property] > b[property]) ? 1 : ((b[property] > a[property]) ? -1 : 0));
  return arr;
};
...
------------------------------------------------------------------------------------------------ */
// got some help from the internet
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < board.length; i++) {
    if (helpCheck(0, i, 1, i, 2, i)) return true;
  }
